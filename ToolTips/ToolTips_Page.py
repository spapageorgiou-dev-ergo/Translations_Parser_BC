    ...
    for root, _, files in os.walk(project_dir):
      ...
      # ...existing code...
                for line in lines:
                    ...
                    if caption_match and id_number and object_type and object_type.lower() == "page":
                        caption_value = caption_match.group(1)
                        # Find the English key that matches this caption value
                        english_key = next(
                            (k for k in translations_dict
                            if k.startswith(f"N{id_number}-") and "P8631-A1033-" in k and caption_value in translations_dict[k]),
                            None
                        )
                        if english_key:
