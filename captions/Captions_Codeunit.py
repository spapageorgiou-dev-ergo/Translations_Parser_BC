...
def inject_translations_to_captions(translations_dict, lang_code,project_dir):
    ...
    caption_pattern = re.compile(r"^\s*Caption\s*=\s*'([^']+)'\s*(;|,)?")
    for root, _, files in os.walk(project_dir):
      ...
      # ...existing code...
                for line in lines:
                    ...
                    if caption_match and id_number and object_type and object_type.lower() == "codeunit":
                        caption_value = caption_match.group(1)
                        # Find the English key that matches this caption value
                        english_key = next(
                            (k for k in translations_dict
                            if k.startswith(f"C{id_number}-") and "P8629-A1033-" in k and caption_value in translations_dict[k]),
                            None
                        )
                        if english_key:
                                ...
                                if idx < len(translations_dict[greek_key]):
                                    ...
                                    if "Comment =" not in line:
                                        inject = caption_pattern.sub(
                                            rf"Caption = '\1' , Comment = '{lang_code}={greek_value}';", line
                                        )
